function LimitSearchX

% LimitSearch runs a limited-exposure search task
%
% Authors: David E. Fencsik
% $LastChangedDate$

experiment = 'LimitSearchPilot01';
Version = '$Rev$';
...
targetList = 1; %[0 1];
setSizeList = [4 10 16];
durationList = [.21 .45 .75];
noiseLevelList = 0;

%%% taskflag values:
%%% 1 = orientation singleton-detection search
%%% 2 = orientation direction-identification search
%%% 3 = color-orientation conjunction singleton-detection search
%%% 4 = color-orientation conjunction category-identification search
%%% 5 = T vs. L target-detection search
%%% 6 = T vs. L direction-identification search
%%% 7 = 2 vs. 5 target-detection search
...
pedestalFlag    = 0; % toggles pedestals
pedestalShape   = 2; % 1 = square, 2 = circle
precueFlag      = 0; % toggles precues
allstimFlag     = 0; % 0 = only show S stim, 1 = always fill all cue locations
stimLocType     = 0; % 0 = random locations, 1 = symmetric locations, 2 = grouped locations
noiseFlag       = 0; % toggles noise
...
respSignalFlag  = 1; % toggles response signal
...
blocktime = now;
Version = Version(7:length(Version)-2); % strip extra text from version string generated by Subversion
...
try
   ...
   refreshDuration = Screen('GetFlipInterval', winMain, 100);
   ...
   elseif taskflag == 1
      ...
      target.texture = Screen('MakeTexture', winMain, mat);
      distractor.texture = stimTextureT;
      target.angle = 20;
      distractor.angle = 0;
      ...
      clear mat;
   elseif taskflag == 5 || taskflag == 6
      % T vs. L
      scolor = colBlue;
      winT = Screen('OpenOffscreenWindow', winMain, colBackground, [0 0 pedestalSize pedestalSize]);
      Screen('FillRect', winT, scolor, [edgeOffset, edgeOffset, pedestalSize - edgeOffset, edgeOffset + stimWidth]);
      Screen('FillRect', winT, scolor, [middle - stimRadius, edgeOffset, middle + stimRadius, pedestalSize - edgeOffset]);
      winL = Screen('OpenOffscreenWindow', winMain, colBackground, [0 0 pedestalSize pedestalSize]);
      Screen('FillRect', winL, scolor, [edgeOffset, edgeOffset, edgeOffset + stimWidth, pedestalSize - edgeOffset]);
      Screen('FillRect', winL, scolor, ...
             [edgeOffset, pedestalSize - edgeOffset - stimWidth, pedestalSize - edgeOffset, pedestalSize - edgeOffset]);

      if taskflag == 5
         taskString = 'detectTvL';
         stimMode = mdDetect;
         responseString0 = 'absent';
         responseString1 = 'present';
         
         target = struct('texture', {0, 0, 0, 0}, 'angle', {0 90, 180, 270}, 'name', {'T0', 'T90', 'T180', 'T270'});
         mat = Screen('GetImage', winT);
         for i = 1:numel(target)
            target(i).texture = Screen('MakeTexture', winMain, mat, target(i).angle);
         end
         distractor = struct('texture', {0, 0, 0, 0}, 'angle', {0 90, 180, 270}, 'name', {'L0', 'L90', 'L180', 'L270'});
         mat = Screen('GetImage', winL);
         for i = 1:numel(target)
            distractor(i).texture = Screen('MakeTexture', winMain, mat, distractor(i).angle);
         end
      elseif taskflag == 6
         taskString = 'identTvL';
         stimMode = mdIdentify;
         responseString0 = 'left';
         responseString1 = 'right';
         
         mat = Screen('GetImage', winT);
         target0 = struct('texture', {0}, 'angle', {90}, 'name', {'T-left'});
         target0.texture = Screen('MakeTexture', winMain, mat, target0.angle);
         target1 = struct('texture', {0}, 'angle', {270}, 'name', {'T-right'});
         target1.texture = Screen('MakeTexture', winMain, mat, target1.angle);
         distractor = struct('texture', {0, 0, 0, 0}, 'angle', {0 90, 180, 270}, 'name', {'L0', 'L90', 'L180', 'L270'});
         mat = Screen('GetImage', winL);
         for i = 1:numel(target)
            distractor(i).texture = Screen('MakeTexture', winMain, mat, distractor(i).angle);
   ...
   for subBlock = subBlockList
      ...
      for trial = 1:nTrials
         ...
         if stimLocType == 0
            ...
            error('stimLocType values of %d are not supported', stimLocType);
         end
         cueloc = stimloc(1:ss);
