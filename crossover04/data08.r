### data08.r: fits a max-rule model to the data.
###
### Assumptions: variable variance, unlimited capacity, fixed criterion
###
...
### Prepare data and output
    ...
    output <- array(NA, dim=c(n.subjects, 10, n.stimsets),
...
### Define fit function
    GoodnessOfFit <- function (p, obse) {
        return(-1 * logLikeBinom(obse$nhits, obse$nfa, obse$npos, obse$nneg,
                                 all.setsizes, p[1], p[2], sdslope=p[3],
...
### convert back to data frames
    ...
    data08 <- list(data=data, par=param,
                rsq=with(data,
