...
###
### This version is based on an unlimited-capacity model, one criterion per
### setsize, and collapses across the palmer flag.

FitMaxCritSSE <- function () {
    thisfile <- "FitMaxCritSSE.r"
    infile <- "data00.rda"
    outfile <- "FitMaxCritSSE.rda"
...
### Fit function
    GoodnessOfFit <- function (p, obs) {
        pred <- maxrule(p[1], p[2:(nSetsizes+1)], setsizes)
...
### Actual fitting
    ...
    for (sub in subjects) {
        for (cond in stimsets) {
            p <- c(3, rep(2, nSetsizes)) # sensitivity and criterion
