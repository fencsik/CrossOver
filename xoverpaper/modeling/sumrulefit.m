function [S,C,SSE]=sumrulefit(data, params, options)
%
% [S,C,SSE]=SUMRULEFIT(DATA,START_PARAMS,OPTIONS)
% Where S C that best fits the data for the specified sum
...
%
% SUMRULEFIT is an attempt to find the sensitivity and criterion for each
...
[R,LogL,exitflag] = fminsearchbnd('sumruleSSE2',pinit,LB,UB,opts,data);

if nargout == 0
   % if no output arguments, then just print the results
   fprintf('S = \n'); S=R(1); disp(S);
   fprintf('C = \n'); C=R(2); disp(C);
   fprintf('SSE = \n'); SSE=sumruleSSE2(R,data); disp(SSE);
elseif nargout == 1
   S=R(1);
elseif nargout == 2
   S=R(1);
