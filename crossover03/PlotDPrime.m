...
% of setsize.
% 
% Reads in data from one file output by the CrossoverSDT7 code, then
...
for nsub = 1:length(Subjects)
   ...
   for ncond = 1:length(Conditions)
      ...
      fprintf('Sub   Cond     SetSize   Pos  Neg   Hits  TNegs  HitRate  FARate   d''    95%%-CI\n');
      for s = 1:length(SetSizes)
         fprintf('%-6s%-10s%5.0f%7.0f%5.0f%7.1f%7.1f%8.2f%8.2f%7.2f%8.2f\n', ...
                 sub, cond, SetSizes(s), ...
                 npos(s), nneg(s), cpos(s), cneg(s), ...
                 hr(s), fa(s), dprime(s), ci(s));
      end
      % plot dprimes
      plot(x, dprime, [colors(counter), '-' points(counter)], ...
      ...
      offset = 0.;
      for s = 1:length(SetSizes)
         plot([x(s) x(s)], [dprime(s) + ci(s), dprime(s) - ci(s)], [colors(counter), '-']);
         plot([x(s)-offset, x(s)+offset], repmat(dprime(s) + ci(s), 1, 2), [colors(counter), '-']);
         plot([x(s)-offset, x(s)+offset], repmat(dprime(s) - ci(s), 1, 2), [colors(counter), '-']);
      end
      plot(x, dprime, [colors(counter), '-' points(counter)], ...
           'LineWidth', 2, 'MarkerSize', 8, 'MarkerFaceColor', 'w');
      text(8.25, dprime(length(SetSizes)), sprintf('%s', cond), ...
